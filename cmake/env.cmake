...
function(_build_detect_arch_from_compiler out)
  # MSVC / clang-cl don't support the GNU-style `-dM -E -x c++ -` probe below,
  # so derive the target arch from CMake's compiler architecture id instead.
  if(CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")
    string(TOLOWER "${CMAKE_CXX_COMPILER_ARCHITECTURE_ID}" _msvc_arch)
    if(_msvc_arch MATCHES "^(x64|x86_64|amd64)$")
      set(${out} "x86_64" PARENT_SCOPE)
    elseif(_msvc_arch MATCHES "^(x86|win32|i386)$")
      set(${out} "x86" PARENT_SCOPE)
    elseif(_msvc_arch STREQUAL "arm64")
      set(${out} "arm64" PARENT_SCOPE)
    elseif(_msvc_arch STREQUAL "arm")
...
function(build_write_abi_json OUT_DIR)
  ...
  # CMAKE_BUILD_TYPE is empty for multi-config generators; fall back to the
  # BUILD_TYPE env var the builder sets from the manifest's code_gen.build_type,
  # so Debug and Release get distinct ABI fingerprints.
  set(_bt "${CMAKE_BUILD_TYPE}")
  if(NOT _bt AND NOT "$ENV{BUILD_TYPE}" STREQUAL "")
    set(_bt "$ENV{BUILD_TYPE}")
  endif()
  if(_bt)
    string(APPEND _json "  \"buildType\": \"${_bt}\",\n")
  else()
  ...
  # exceptions / rtti from the env vars the builder sets from the manifest's
  # language settings (CMAKE_CXX_EXCEPTIONS/RTTI aren't standard CMake vars).
  if(NOT "$ENV{BUILD_EXCEPTIONS}" STREQUAL "")
    string(APPEND _json "  \"exceptions\": $ENV{BUILD_EXCEPTIONS},\n")
  else()
    string(APPEND _json "  \"exceptions\": true,\n")
  endif()
  if(NOT "$ENV{BUILD_RTTI}" STREQUAL "")
    string(APPEND _json "  \"rtti\": $ENV{BUILD_RTTI},\n")
  else()
